...
var mouseRayFollower = new THREE.Mesh(new THREE.TorusGeometry(2.5, 0.1, 16, 100), new THREE.MeshBasicMaterial({ color: 0x00ff00, transparent: true, opacity: 0.15 }));
...
var weaponRange = 500, weaponPosition, weaponRotation, weaponRealWorldScaleMultiplier, weaponPlacementOffset, weaponScale, weaponHelpText, weaponType, rateOfFire, // time in ms in between attacks()
...
class blockData {
    constructor(value, position, chunk, chunkIndex) {
        this.value = value;
...
class DiscreteVectorField {
    constructor() {
        ...
        this.field = []; // stores the value of each voxel
        this.field = [];
        this.chunkIndexes = {};
    }
    ...
    set(x, y, z, value, chunkIndex, chunk) {
        ...
        if (!this.field[x][y][z]) {
            this.field[x][y][z] = new blockData(value, new THREE.Vector3(x, y, z), chunk, chunkIndex);
        }
    }
...
const generateWorld = function (modelURL) {
    ...
    ipcRenderer.on('list-maps-reply', (event, arg) => {
        ...
        ipcRenderer.on('get-map-metadata-reply', (event, arg) => {
            ...
            if (groundData) {
                const groundFloor = new THREE.Mesh(new THREE.BoxGeometry(groundSize.x, 1, groundSize.y), new THREE.MeshBasicMaterial({
...
const buildWorldModelFromBox = function (scale, position, material, color) {
    ...
    const resetChunkBounds = function () {
        // For every chunk ...
        chunkMinPosition = new THREE.Vector3(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
        chunkMaxPosition = new THREE.Vector3(Number.MIN_SAFE_INTEGER, Number.MIN_SAFE_INTEGER, Number.MIN_SAFE_INTEGER);
    };
    resetChunkBounds();
    let instancedWorldModel = new VoxelChunk(voxelGeometry, voxelMaterial, maxChunkSize);
    ...
    const finalizeChunk = function () {
        ...
        // Determine Center
        new THREE.Vector3((chunkMinPosition.x + chunkMaxPosition.x) / 2, (chunkMinPosition.y + chunkMaxPosition.y) / 2, (chunkMinPosition.z + chunkMaxPosition.z) / 2), 
        // Determine Size
        new THREE.Vector3((chunkMaxPosition.x - chunkMinPosition.x), (chunkMaxPosition.y - chunkMinPosition.y), (chunkMaxPosition.z - chunkMinPosition.z)));
        ...
        chunkCounter++;
        instancedWorldModel = new VoxelChunk(voxelGeometry, voxelMaterial, maxChunkSize);
        instancedWorldModel.name = chunkCounter.toString();
    };
    ...
    for (let x = 0; x < scale.x; x++) {
        for (let z = 0; z < scale.z; z++) {
            setVoxel(new THREE.Vector3(position.x + x, position.y, position.z + z));
        }
    }
    finalizeChunk();
    ...
    for (let x = 0; x < scale.x; x++) {
        for (let z = 0; z < scale.z; z++) {
            setVoxel(new THREE.Vector3(position.x + x, position.y + scale.y, position.z + z));
        }
    }
    finalizeChunk();
    ...
    for (let y = 0; y < scale.y; y++) {
        for (let z = 0; z < scale.z; z++) {
            setVoxel(new THREE.Vector3(position.x, position.y + y, position.z + z));
            setVoxel(new THREE.Vector3(position.x + scale.x, position.y + y, position.z + z));
        }
    }
    finalizeChunk();
    for (let y = 0; y < scale.y; y++) {
        for (let x = 0; x < scale.x; x++) {
            setVoxel(new THREE.Vector3(position.x + x, position.y + y, position.z));
            setVoxel(new THREE.Vector3(position.x + x, position.y + y, position.z + scale.z));
        }
    }
    finalizeChunk();
};
...
const generateWeaponModel = function (modelURL) {
    ...
    loader.load(modelURL, function (jsondata) {
        let jsonModel = JSON.parse(jsondata);
        let voxelPositionsFromFile = jsonModel.voxels;
        instancedWeaponModel = new VoxelChunk(voxelGeometry, weaponModelMaterial, voxelPositionsFromFile.length);
        ...
        instancedWeaponTarget = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshStandardMaterial({ color: 0xff0000 }));
        ...
        if (jsonModel.weaponData.placementOffset)
            weaponPlacementOffset = new THREE.Vector3(jsonModel.weaponData.placementOffset.x, jsonModel.weaponData.placementOffset.y, jsonModel.weaponData.placementOffset.z);
        ...
        if (jsonModel.weaponData.realWorldScaleMultiplier != undefined)
            weaponRealWorldScaleMultiplier = jsonModel.weaponData.realWorldScaleMultiplier;
...
const createTrivoxelAt = function (x, y, z, color) {
    ...
    new THREE.BoxGeometry(1, 1, 1), new THREE.MeshStandardMaterial({ color: color }));
    ...
    const triVoxelVelocity = new THREE.Vector3((Math.random() * velocityRange / dropAngle) - (velocityRange / dropAngle), (Math.random() * velocityRange / (dropAngle / 5)) - (velocityRange / (dropAngle / 5)), (Math.random() * velocityRange / dropAngle) - (velocityRange / dropAngle));
...
const createParticleInstance = function (effect, worldPos) {
    ...
    for (let i = 0; i < effect.count; i++) {
        particlePositions.push((Math.random() * effect.spread.x) - (effect.spread.x / 2), (Math.random() * effect.spread.y) - (effect.spread.y / 2), (Math.random() * effect.spread.z) - (effect.spread.z / 2));
        particleVelocities.push(effect.direction.x * (Math.random() + 0.5), effect.direction.y * (Math.random() + 0.5), effect.direction.z * (Math.random() + 0.5));
...
document.addEventListener('mousedown', (e) => { if (e.button != 0)
    return; isLeftClicking = true; });
document.addEventListener('mouseup', (e) => { if (e.button != 0)
    return; isLeftClicking = false; });
// MUZZLE FLASH (for weapons)
const muzzleFlash = new THREE.Mesh(new THREE.ConeGeometry(5, 75, 8, 1), new THREE.MeshBasicMaterial({
    color: 0xffd6c4
}));
...
const render = function () {
    ...
    document.querySelector("#position-overlay").innerHTML = ("<font color='red'>X: " + Math.round(controls.getObject().position.x) +
    ...
    if (instancedWeaponModel && instancedWeaponTarget && delta > 0) {
        ...
        if (!defaultInstancedWeaponTargetPosition)
            defaultInstancedWeaponTargetPosition = instancedWeaponTarget.position.clone();
        ...
        switch (weaponType) {
            ...
            case "ranged":
                if (isAttackAvailable) {
                    ...
                    instancedWeaponTarget.position.set(weaponStartPosition.x + Math.random() * bzzStrength - bzzStrength / 2 - 0.5, weaponStartPosition.y + Math.random() * bzzStrength - bzzStrength / 2 + 0.5, weaponStartPosition.z + Math.random() * bzzStrength - bzzStrength / 2 + 0.5);
...
const physicsUpdate = function () {
    ...
    triVoxelDroppedPieces.forEach((triVoxel) => {
        if (Math.random() < 0.1 / physicsUpdatesPerSecond) {
            ...
            const newPosition = new THREE.Vector3(sceneObject.position.x + velocity.x, sceneObject.position.y + velocity.y, sceneObject.position.z + velocity.z);
            ...
            if (Math.random() < 0.000001 / physicsUpdatesPerSecond) {
                ...
                for (let i = 0; i < cubeDestructionParticles.length; i += 3) {
                    if (Math.random() < 1 / physicsUpdatesPerSecond) {
                        ...
                        // get the position of the first matrix object
                        const position = new THREE.Vector3(cubeDestructionParticles[i], cubeDestructionParticles[i + 1], cubeDestructionParticles[i + 2]);
        ...
        wreckedMeshes.forEach(wreckedMesh => {
            ...
            for (let i = 0; i < wreckedMesh.count; i++) {
                ...
                if (position.y > 1) {
                    ...
                    if (!wreckedMesh.velocities[i].bounceCount)
                        wreckedMesh.velocities[i].bounceCount = 1;
...
const createVizSphere = function (x, y, z, color = 0x00ff00, size = 1.5) {
    const sphere = new THREE.Mesh(new THREE.SphereGeometry(size, 2, 2), new THREE.MeshBasicMaterial({ color: color }));
    ...
    if (confirmedTouch.length == holeBorder.length) {
        ...
        const hasAYPosition = function (x, z) {
            ...
            for (let i = 0; i < holeBorder.length; i++) {
                if (holeBorder[i].x == x && holeBorder[i].z == z)
                    count++;
                if (count >= 2)
                    return true;
            }
        ...
        if (USERSETTINGS.useSpriteParticles) {
            ...
            for (let i = 0; i < fullHole.length; i++) {
                ...
                cubeDestructionParticles.velocities.push(new THREE.Vector3((Math.random() - 0.5) * velocityStrength, -(Math.random() + 0.5) * (velocityStrength * 5), (Math.random() - 0.5) * velocityStrength));
            }
            ...
            const wreckedMesh = new VoxelChunk(voxelGeometry, voxelMaterial, fullHole.length);
            ...
            for (let i = 0; i < fullHole.length; i++) {
                ...
                wreckedMesh.velocities.push(new THREE.Vector3((Math.random() - 0.5) * velocityStrength, -(Math.random() + 0.5) * velocityStrength, (Math.random() - 0.5) * velocityStrength));
...
const shootRay = function (emitParticleEffect) {
    ...
    if (intersection != null) {
        ...
        const intersectPosition = new THREE.Vector3(intersection.x, intersection.y, intersection.z);
        if (emitParticleEffect) {
            ...
            /*randomSpriteRange:*/ 3, 
            /*useGravity:*/ false, 
            /*fadeOut:*/ true), intersectPosition);
        }
        ...
        for (let x = intersectPosition.x - destroyedChunkRange; x <= intersectPosition.x + destroyedChunkRange; x++) {
            for (let y = intersectPosition.y - destroyedChunkRange; y <= intersectPosition.y + destroyedChunkRange; y++) {
                for (let z = intersectPosition.z - destroyedChunkRange; z <= intersectPosition.z + destroyedChunkRange; z++) {
                    ...
                    if ((distanceToIntersectPos * percentMissedHits) < rapidFloat())
        ...
        if (recentlyEditedWorldModels.length > maxChunksInPhysicsCache)
            recentlyEditedWorldModels.shift();
...
const plantExplosive = function () {
    ...
    if (intersection != null) {
        ...
        const intersectionPosition = new THREE.Vector3(intersection.x, intersection.y, intersection.z);
        ...
        for (let x = -(damageRange / 2); x <= (damageRange / 2); x++) {
            for (let y = -(damageRange / 2); y <= (damageRange / 2); y++) {
                for (let z = -(damageRange / 2); z <= (damageRange / 2); z++) {
                    const voxelPosition = new THREE.Vector3(x + position.x, y + position.y, z + position.z);
                    const voxel = voxelField.get(voxelPosition.x, voxelPosition.y, voxelPosition.z);
                    if (voxel.value != 0) {
